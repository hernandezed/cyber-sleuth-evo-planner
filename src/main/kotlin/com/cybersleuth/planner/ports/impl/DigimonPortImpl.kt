...
class DigimonMapper {
    fun map(digimon: Digimon): DigimonBo {
        return DigimonBo(digimon.id, digimon.name, SkillBo(digimon.skill.name, digimon.skill.description), digimon.stage, digimon.type, digimon.attribute, digimon.memory, digimon.slots, digimon.evolveFrom.map { DegenerationBo(it.id, it.name) }.toSet(), digimon.evolveTo.map { EvolutionBo(it.to.id, it.to.name, RequirementBo(it.requirements.level, it.requirements.hp, it.requirements.sp, it.requirements.atk, it.requirements.def, it.requirements.int, it.requirements.spd, it.requirements.cam, it.requirements.abi, it.requirements.dna)) }.toSet(), digimon.learnedAttacks.map { LearnAttackBo(AttackBo(it.learnedAttack.id, it.learnedAttack.name, it.learnedAttack.attribute, it.learnedAttack.type, it.learnedAttack.cost, it.learnedAttack.power, it.learnedAttack.inheritable), it.at) }.toSet(),
